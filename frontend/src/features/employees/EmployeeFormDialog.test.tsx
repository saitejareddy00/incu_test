...
describe('EmployeeFormDialog — create mode', () => {
  ...
  it('calls POST with salary in cents and closes dialog on valid submit', async () => {
    let postedBody: Record<string, unknown> | undefined;
    server.use(
      http.post('/api/employees', async ({ request }) => {
        postedBody = (await request.json()) as Record<string, unknown>;
        return HttpResponse.json({ id: 'new-1', ...postedBody }, { status: 201 });
      }),
    );

    const onClose = vi.fn();
    wrap(<EmployeeFormDialog open onClose={onClose} />);
    await fillForm({ salaryDollars: 120000 });
    await userEvent.click(screen.getByRole('button', { name: /save/i }));
...
describe('EmployeeFormDialog — edit mode', () => {
  ...
  it('pre-populates annual salary from cents', () => {
    wrap(<EmployeeFormDialog open onClose={vi.fn()} employee={existingEmployee} />);
