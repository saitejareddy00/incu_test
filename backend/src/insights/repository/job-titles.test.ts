...
describe('listJobTitles', () => {
  ...
  it('filters job titles by country when provided', async () => {
    await withTestDb(async (client) => {
      await createEmployee(client, { ...base, email: 'a@example.com', jobTitle: 'Engineer', country: 'US' });
      await createEmployee(client, { ...base, email: 'b@example.com', jobTitle: 'Manager', country: 'GB' });
