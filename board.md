...
### US-002 · docker-compose.yml (Postgres + app_test DB)
...
### US-005 · Curated first_names.txt / last_names.txt (~500 each)
...
### US-102 · SQL migration runner (.sql files + schema_migrations)
...
- A `schema_migrations` table (filename TEXT PK, applied_at TIMESTAMPTZ) is created on first run.
...
- Indexes exist on `(country)`, `(country, job_title)`, `(job_title)`, and a pg_trgm trigram index on `full_name`.
...
_Last updated: US-803 done — EPIC-8 complete (performance.md added)._
